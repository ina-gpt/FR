...
function classify(rel, line) {
  ...
  if (
    /\/(api|v[0-9])\/[a-z0-9/_-]*(ollama|qwen|whisper|piper|nomic)/i.test(line) ||
    /\b[A-Z][A-Z0-9_]*(OLLAMA|QWEN|WHISPER|PIPER|NOMIC)[A-Z0-9_]*\b/.test(line) ||
    /["'`][a-z0-9._/-]*(ollama|qwen|whisper|piper|nomic|deepseek)[a-z0-9._:/-]*["'`]/i.test(line)
  ) return 'ID';
  return 'CODE';
}
...
if (MODE_STDIN) {
  ...
  try {
    ...
    // for an empty repo.
    files = execFileSync('git', ['-C', REPO, ...cmd], {
      encoding: 'utf8',
...
if (MODE_JSON) {
  ...
  process.stdout.write(`brand-lint: ${scanned} file(s) scanned, ${findings.length} violation(s)\n`);
}
